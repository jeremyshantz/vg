...
TEST(writeXml, XmlWriter)
{
    ...
    Xml::HElement t = Xml::Reader::loadXml(xmlStream);
    
    std::stringstream xmlOut;
    Xml::Writer::writeXml(t, xmlOut);
    ...
    // Just reload the xml and run the tests from SceneWriterTests
    
    // Parse the XML stream and create an XML "DOM"
   Xml::HElement readRoot = Xml::Reader::loadXml(xmlOut);
